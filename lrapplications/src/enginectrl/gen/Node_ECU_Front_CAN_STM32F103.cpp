...
int8_t createMsg_EngineCtrlFront_Alive(CAN_FRAME* pCANFrame, Msg_EngineCtrlFront_Alive* pMsg)
{
	...
	// Start-Bit: 0 End-Bit: 15	pCANFrame->data[0] = (uint8_t)((pMsg->EngineCtrlFront_Alive & 0xFF00) >> 8);
...
int8_t createMsg_Engine_Speed_Front(CAN_FRAME* pCANFrame, Msg_Engine_Speed_Front* pMsg)
{
	...
	// Start-Bit: 0 End-Bit: 15	pCANFrame->data[0] = (uint8_t)((pMsg->Engine_Speed_F_L & 0xFF00) >> 8);
	pCANFrame->data[1] = (uint8_t)(pMsg->Engine_Speed_F_L & 0x00FF);
	// Signal	: Engine_Speed_F_R
	// Start-Bit: 16 End-Bit: 31	pCANFrame->data[2] = (uint8_t)((pMsg->Engine_Speed_F_R & 0xFF00) >> 8);
...
int8_t createMsg_Wheel_Speed_Front(CAN_FRAME* pCANFrame, Msg_Wheel_Speed_Front* pMsg)
{
	...
	// Start-Bit: 0 End-Bit: 15	pCANFrame->data[0] = (uint8_t)((pMsg->Wheel_Speed_F_L & 0xFF00) >> 8);
	pCANFrame->data[1] = (uint8_t)(pMsg->Wheel_Speed_F_L & 0x00FF);
	// Signal	: Wheel_Speed_F_R
	// Start-Bit: 16 End-Bit: 31	pCANFrame->data[2] = (uint8_t)((pMsg->Wheel_Speed_F_R & 0xFF00) >> 8);
...
int8_t parseMsg_Wheel_Speed_Front_Setpoint(CAN_FRAME* pCANFrame, Msg_Wheel_Speed_Front_Setpoint* pMsg)
{
	...
	// Extract the data from CAN-Frame	// Signal	: Wheel_Speed_F_L_Setpoint
...
int8_t isMsg_Wheel_Speed_Front_Setpoint(CAN_FRAME* pCANFrame)
{
	uint32_t canID = 0;
