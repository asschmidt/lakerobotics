...
#include "mcp2515/can.h"
  ...
  *
  */
	int8_t createMsg_PowerManager_Alive(struct can_frame* pCANFrame, Msg_PowerManager_Alive* pMsg)
	{
		pCANFrame->can_id = CAN_ID_PowerManager_Alive;
		pCANFrame->can_dlc = 2;
		// Signal	: PowerManager_Alive
		// Start-Bit: 0 End-Bit: 15		pCANFrame->data[0] = (int8_t)((pMsg->PowerManager_Alive & 0xFF00) >> 8);
		pCANFrame->data[1] = (int8_t)(pMsg->PowerManager_Alive & 0x00FF);
	...
	int8_t createMsg_Power_Supply_Motor(struct can_frame* pCANFrame, Msg_Power_Supply_Motor* pMsg)
	{
		...
		// Start-Bit: 0 End-Bit: 15		pCANFrame->data[0] = (int8_t)((pMsg->Power_Supply_Motor_Voltage & 0xFF00) >> 8);
		pCANFrame->data[1] = (int8_t)(pMsg->Power_Supply_Motor_Voltage & 0x00FF);
		// Signal	: Power_Supply_Motor_Current
		// Start-Bit: 16 End-Bit: 31		pCANFrame->data[2] = (int8_t)((pMsg->Power_Supply_Motor_Current & 0xFF00) >> 8);
	...
	int8_t createMsg_Power_Supply_Electronic(struct can_frame* pCANFrame, Msg_Power_Supply_Electronic* pMsg)
	{
		...
		// Start-Bit: 0 End-Bit: 15		pCANFrame->data[0] = (int8_t)((pMsg->Power_Supply_Electronic_Voltage & 0xFF00) >> 8);
		pCANFrame->data[1] = (int8_t)(pMsg->Power_Supply_Electronic_Voltage & 0x00FF);
		// Signal	: Power_Supply_Electronic_Current
		// Start-Bit: 16 End-Bit: 31		pCANFrame->data[2] = (int8_t)((pMsg->Power_Supply_Electronic_Current & 0xFF00) >> 8);
  ...
  */
	int8_t parseMsg_Power_Control(struct can_frame* pCANFrame, Msg_Power_Control* pMsg)
	{
		...
		if (pCANFrame->can_id != CAN_ID_Power_Control)			return -1;

		// Check DLC
		if (pCANFrame->can_dlc != 4)
			return -1;

		// Extract the data from CAN-Frame		// Signal	: Power_Electronic_1_Active
	...
	int8_t isMsg_Power_Control(struct can_frame* pCANFrame)
	{
		// Check the Standard ID
