...
{
	pCANFrame->txHeader.ExtId = CAN_ID_EngineCtrlMid_Alive;
	...
	pCANFrame->data[0] = (uint8_t)((pMsg->EngineCtrlMid_Alive & 0xFF00) >> 8);
...
{
	pCANFrame->txHeader.ExtId = CAN_ID_Engine_Speed_Mid;
	pCANFrame->txHeader.StdId = 0;
	pCANFrame->txHeader.IDE   = CAN_ID_EXT;
	...
	// Start-Bit: 0 End-Bit: 15
	pCANFrame->data[0] = (uint8_t)((pMsg->Engine_Speed_M_L & 0xFF00) >> 8);
	pCANFrame->data[1] = (uint8_t)(pMsg->Engine_Speed_M_L & 0x00FF);
	// Signal	: Engine_Speed_M_R
	// Start-Bit: 16 End-Bit: 31
	pCANFrame->data[2] = (uint8_t)((pMsg->Engine_Speed_M_R & 0xFF00) >> 8);
...
{
	pCANFrame->txHeader.ExtId = CAN_ID_Wheel_Speed_Mid;
	pCANFrame->txHeader.StdId = 0;
	pCANFrame->txHeader.IDE   = CAN_ID_EXT;
	...
	// Start-Bit: 0 End-Bit: 15
	pCANFrame->data[0] = (uint8_t)((pMsg->Wheel_Speed_M_L & 0xFF00) >> 8);
	pCANFrame->data[1] = (uint8_t)(pMsg->Wheel_Speed_M_L & 0x00FF);
	// Signal	: Wheel_Speed_M_R
	// Start-Bit: 16 End-Bit: 31
	pCANFrame->data[2] = (uint8_t)((pMsg->Wheel_Speed_M_R & 0xFF00) >> 8);
...
int8_t parseMsg_Wheel_Speed_Mid_Setpoint(CAN_FRAME* pCANFrame, Msg_Wheel_Speed_Mid_Setpoint* pMsg)
{
	...
	// Signal	: Wheel_Speed_M_L_Setpoint
	// Start-Bit: 0 End-Bit: 15
